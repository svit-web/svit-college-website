...
-- 
-- This is the complete public schema captured from dev Supabase (agezrfclusigfqysbxwb)
-- on 2026-08-12 before beginning the Next.js migration.
--
-- Purpose: Provides a clean rollback point. If the migration encounters schema issues,
-- this baseline allows resetting to a known-good state.
--
-- Generated via: pg_dump --schema-only --schema=public --no-owner
-- Counts: 52 tables, 187 RLS policies, 13 functions, 51 triggers
-- 
-- NOTE: Dev and prod schemas match except for homepage_items.pretitle column
-- which exists on dev but not prod. The prod branch does not reference it.
