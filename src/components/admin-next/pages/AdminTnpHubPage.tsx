...
export function AdminTnpHubPage() {
  ...
  const [recruiterForm, setRecruiterForm] = useState<RecruiterItem>({ id: '', companyName: '', logo: null });

  const openAddRecruiter = () => {
    setEditingRecruiterId(null);
    setRecruiterForm({ id: `r_${Date.now()}`, companyName: '', logo: null });
    ...
    <div className="max-w-5xl space-y-6 pb-20">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-slate-200 pb-4">
        ...
        <div className="flex items-center gap-2">
          <a href="/placement" target="_blank" rel="noreferrer" className="inline-flex items-center gap-1.5 rounded-lg border border-slate-200 bg-white px-3 py-1.5 text-xs font-semibold text-slate-700 hover:bg-slate-50 transition">
        ...
        {[
          ...
          return (
            <button
              ...
              className={`inline-flex items-center gap-1.5 rounded-lg px-3 py-1.5 text-xs font-bold transition ${isActive ? 'bg-navy text-white shadow-xs' : 'bg-slate-100 text-slate-600 hover:bg-slate-200 hover:text-slate-900'}`}
            >
      ...
      {activeTab === 'toggles' && (
        <div className="rounded-xl border border-slate-200 bg-white p-5 space-y-4">
          <div>
            ...
            <p className="text-xs text-slate-500 mt-0.5">Toggle any of the body sections on or off. Disabled sections will be hidden on the public /placement page without code changes.</p>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 pt-2">
            {[
              ...
              return (
                <div key={s.key} className={`flex items-start justify-between rounded-xl border p-4 transition ${isEnabled ? 'border-navy/30 bg-navy/5' : 'border-slate-200 bg-slate-50 opacity-60'}`}>
                  ...
                  <button
                    ...
                    className={`relative inline-flex h-6 w-11 shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out ${isEnabled ? 'bg-navy' : 'bg-slate-300'}`}
                  >
        ...
        <div className="rounded-xl border border-slate-200 bg-white p-5 space-y-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div className="space-y-1">
              ...
              <input type="text" value={data.heroTitle} onChange={(e) => setData({ ...data, heroTitle: e.target.value })} className="w-full rounded-lg border border-slate-300 px-3 py-2 text-xs text-slate-800 focus:border-navy focus:outline-none" />
            </div>

            <div className="space-y-1">
              <label className="text-xs font-semibold text-slate-700">Hero Subtitle</label>
              <input type="text" value={data.heroSubtitle} onChange={(e) => setData({ ...data, heroSubtitle: e.target.value })} className="w-full rounded-lg border border-slate-300 px-3 py-2 text-xs text-slate-800 focus:border-navy focus:outline-none" />
            </div>
      ...
      {activeTab === 'about' && (
        <div className="space-y-5">
          <div className="rounded-xl border border-slate-200 bg-white p-5 space-y-3">
            ...
            <textarea rows={4} value={data.aboutText} onChange={(e) => setData({ ...data, aboutText: e.target.value })} className="w-full rounded-lg border border-slate-300 p-3 text-xs text-slate-800 focus:border-navy focus:outline-none" />
          </div>
          ...
          <div className="rounded-xl border border-slate-200 bg-white p-5 space-y-4">
            ...
            <div className="flex flex-wrap items-center gap-2 bg-slate-50 p-3 rounded-lg border border-slate-200">
              <select value={newHighlightIcon} onChange={(e) => setNewHighlightIcon(e.target.value)} className="rounded-lg border border-slate-300 px-2 py-1.5 text-xs text-slate-800 focus:outline-none bg-white">
              ...
              <button type="button" onClick={addHighlight} className="inline-flex items-center gap-1 rounded-lg bg-navy px-3 py-1.5 text-xs font-bold text-white hover:bg-navy/90 transition">
            ...
            <div className="space-y-2">
              {(data.sectionConfig?.highlights || []).map((h) => (
                <div key={h.id} className="flex items-center justify-between rounded-lg border border-slate-200 bg-white p-3">
                  <div className="flex items-center gap-2">
                    <span className="rounded bg-navy/10 px-2 py-0.5 text-[10px] font-bold text-navy">{h.icon}</span>
                    <span className="text-xs font-semibold text-slate-800">{h.label}</span>
                  </div>
                  <button type="button" onClick={() => deleteHighlight(h.id)} className="text-rose-500 hover:text-rose-700 p-1">
        ...
        <div className="rounded-xl border border-slate-200 bg-white p-5 space-y-4">
          ...
          <div className="flex flex-wrap items-center gap-3 bg-slate-50 p-3 rounded-lg border border-slate-200">
            <div className="space-y-1">
              ...
              <input type="text" value={newYear} onChange={(e) => setNewYear(e.target.value)} placeholder="2026" className="w-20 rounded-lg border border-slate-300 px-2 py-1 text-xs text-slate-800 bg-white" />
            </div>
            <div className="space-y-1">
              <label className="text-[10px] font-bold uppercase text-slate-500">Students Placed</label>
              <input type="number" value={newPlaced} onChange={(e) => setNewPlaced(e.target.value)} placeholder="220" className="w-28 rounded-lg border border-slate-300 px-2 py-1 text-xs text-slate-800 bg-white" />
            </div>
            <div className="space-y-1">
              <label className="text-[10px] font-bold uppercase text-slate-500">Placement %</label>
              <input type="number" value={newPct} onChange={(e) => setNewPct(e.target.value)} placeholder="95" className="w-24 rounded-lg border border-slate-300 px-2 py-1 text-xs text-slate-800 bg-white" />
            </div>
            <button type="button" onClick={addTrendYear} className="inline-flex items-center gap-1 rounded-lg bg-navy px-3 py-1.5 text-xs font-bold text-white hover:bg-navy/90 transition mt-4">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-6 gap-3 pt-2">
            {data.graphicalData.map((pt) => (
              <div key={pt.year} className="relative flex flex-col items-center justify-between rounded-xl border border-slate-200 bg-white p-3 text-center">
                <button type="button" onClick={() => deleteTrendYear(pt.year)} className="absolute top-1.5 right-1.5 text-slate-400 hover:text-rose-500">
      ...
      {activeTab === 'students' && (
        <div className="rounded-xl border border-slate-200 bg-white p-5 space-y-4">
          <div className="flex items-center justify-between border-b border-slate-100 pb-3">
            ...
            <button type="button" onClick={openAddStudent} className="inline-flex items-center gap-1.5 rounded-lg bg-navy px-3 py-1.5 text-xs font-bold text-white hover:bg-navy/90 transition">
            ...
            {data.placedStudents.map((s) => (
              <div key={s.id} className="group relative flex flex-col items-center rounded-xl border border-slate-200 bg-white p-3 text-center hover:border-navy transition">
                ...
                <div className="mt-2 flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                  <button type="button" onClick={() => openEditStudent(s)} className="p-1 text-slate-500 hover:text-navy">
                    <Pencil className="h-3 w-3" />
                  </button>
                  <button type="button" onClick={() => deleteStudent(s.id)} className="p-1 text-rose-500 hover:text-rose-700">
                    <Trash2 className="h-3 w-3" />
                  </button>
                </div>
              </div>
      ...
      {activeTab === 'recruiters' && (
        <div className="rounded-xl border border-slate-200 bg-white p-5 space-y-4">
          <div className="flex items-center justify-between border-b border-slate-100 pb-3">
            ...
            <button type="button" onClick={openAddRecruiter} className="inline-flex items-center gap-1.5 rounded-lg bg-navy px-3 py-1.5 text-xs font-bold text-white hover:bg-navy/90 transition">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 md:grid-cols-6 gap-3">
            {data.recruiters.map((r) => (
              <div key={r.id} className="group relative flex flex-col items-center justify-between rounded-xl border border-slate-200 bg-white p-3 text-center hover:border-navy transition">
                <div className="h-10 w-full flex items-center justify-center">
                  {r.logo ? <img src={r.logo} alt={r.companyName} className="max-h-8 max-w-[80%] object-contain" /> : <span className="font-bold text-xs text-navy">{r.companyName}</span>}
                </div>
                <div className="text-xs font-bold text-slate-800 truncate w-full mt-2">{r.companyName}</div>

                <div className="mt-2 flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                  <button type="button" onClick={() => openEditRecruiter(r)} className="p-1 text-slate-500 hover:text-navy">
                    <Pencil className="h-3 w-3" />
                  </button>
                  <button type="button" onClick={() => deleteRecruiter(r.id)} className="p-1 text-rose-500 hover:text-rose-700">
                    <Trash2 className="h-3 w-3" />
                  </button>
                </div>
              </div>
        ...
        <div className="rounded-xl border border-slate-200 bg-white p-5 space-y-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div className="space-y-2">
              ...
              <MediaUploader value={data.officer.photo || ''} onChange={(url) => setData({ ...data, officer: { ...data.officer, photo: url || null } })} type="image" />
            </div>
            ...
            <div className="space-y-3">
              <div className="space-y-1">
                ...
                <input
                  ...
                  onChange={(e) => setData({ ...data, officer: { ...data.officer, name: e.target.value } })}
                  className="w-full rounded-lg border border-slate-300 px-3 py-2 text-xs text-slate-800 focus:border-navy focus:outline-none"
                />
              </div>
              ...
              <div className="space-y-1">
                ...
                <input
                  ...
                  onChange={(e) => setData({ ...data, officer: { ...data.officer, designation: e.target.value } })}
                  className="w-full rounded-lg border border-slate-300 px-3 py-2 text-xs text-slate-800 focus:border-navy focus:outline-none"
                />
              </div>
              ...
              <div className="grid grid-cols-2 gap-2">
                <div className="space-y-1">
                  ...
                  <input
                    ...
                    onChange={(e) => setData({ ...data, officer: { ...data.officer, phone: e.target.value } })}
                    className="w-full rounded-lg border border-slate-300 px-3 py-2 text-xs text-slate-800 focus:border-navy focus:outline-none"
                  />
                </div>
                ...
                <div className="space-y-1">
                  ...
                  <input
                    ...
                    onChange={(e) => setData({ ...data, officer: { ...data.officer, email: e.target.value } })}
                    className="w-full rounded-lg border border-slate-300 px-3 py-2 text-xs text-slate-800 focus:border-navy focus:outline-none"
                  />
                </div>
              </div>
      ...
      {showStudentModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/40 backdrop-blur-xs p-4">
          <div className="w-full max-w-md rounded-2xl bg-white p-6 shadow-xl space-y-4">
            <h3 className="text-base font-bold text-slate-900">{editingStudentId ? 'Edit Placed Student Card' : 'Add Placed Student Card'}</h3>
            ...
            <div className="space-y-3">
              ...
              <div className="grid grid-cols-2 gap-2">
                ...
                <div className="space-y-1">
                  ...
                  <select value={studentForm.collegeId} onChange={(e) => setStudentForm({ ...studentForm, collegeId: e.target.value })} className="w-full rounded-lg border border-slate-300 px-3 py-2 text-xs text-slate-800 bg-white">
              ...
              <div className="space-y-1">
                ...
                <MediaUploader value={studentForm.photo || ''} onChange={(url) => setStudentForm({ ...studentForm, photo: url || null })} type="image" />
              </div>
            </div>

            <div className="flex justify-end gap-2 pt-2">
              <button type="button" onClick={() => setShowStudentModal(false)} className="rounded-lg border border-slate-300 px-4 py-2 text-xs font-semibold text-slate-700 hover:bg-slate-50">
                Cancel
              </button>
              <button type="button" onClick={saveStudent} className="rounded-lg bg-navy px-4 py-2 text-xs font-bold text-white hover:bg-navy/90">
                Save Card
              </button>
            </div>
          </div>
      ...
      {showRecruiterModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/40 backdrop-blur-xs p-4">
          <div className="w-full max-w-md rounded-2xl bg-white p-6 shadow-xl space-y-4">
            <h3 className="text-base font-bold text-slate-900">{editingRecruiterId ? 'Edit Recruiter Logo' : 'Add Recruiter Logo'}</h3>
            ...
            <div className="space-y-3">
              ...
              <div className="space-y-1">
                ...
                <MediaUploader value={recruiterForm.logo || ''} onChange={(url) => setRecruiterForm({ ...recruiterForm, logo: url || null })} type="image" />
              </div>
            </div>

            <div className="flex justify-end gap-2 pt-2">
              <button type="button" onClick={() => setShowRecruiterModal(false)} className="rounded-lg border border-slate-300 px-4 py-2 text-xs font-semibold text-slate-700 hover:bg-slate-50">
                Cancel
              </button>
              <button type="button" onClick={saveRecruiter} className="rounded-lg bg-navy px-4 py-2 text-xs font-bold text-white hover:bg-navy/90">
                Save Recruiter
              </button>
            </div>
          </div>
