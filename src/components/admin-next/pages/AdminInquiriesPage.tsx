...
import { FileText, Download, Search, Plus, Trash2, Edit2, Loader2, Inbox, Settings, Calendar } from 'lucide-react';
...
export function AdminInquiriesPage({ admin }: { admin: AdminUser }) {
  ...
  async function loadForms() {
    ...
    try {
      const { data, error } = await supabase.from('inquiry_forms').select('*').is('deleted_at', null).order('form_name', { ascending: true });

      if (error) throw error;
      setFormsList((data as any) || []);
      if (data && data.length > 0) {
        setSelectedFormId((prev) => prev || data[0].id);
      }
    } catch (err: any) {
  ...
  async function loadSubmissions(formId: string) {
    ...
    try {
      const { data, error } = await supabase.from('inquiry_submissions').select('*').eq('form_id', formId).is('deleted_at', null).order('created_at', { ascending: false });
  ...
  const handleSaveForm = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (editingForm) {
        const { error } = await supabase.from('inquiry_forms').update(payload as any).eq('id', editingForm.id);
        if (error) throw error;
        toast.success('Form definition modified successfully!');
      } else {
        const { error } = await supabase.from('inquiry_forms').insert(payload as any);
        if (error) throw error;
  ...
  const handleExportCSV = () => {
    ...
    const rows = submissions.map((sub) => [sub.id, new Date(sub.created_at).toLocaleString(), ...dataKeys.map((key) => sub.submitted_data?.[key] || ''), sub.notes || '']);
  ...
  const filteredSubmissions = useMemo(() => {
    return submissions.filter((sub) => {
      ...
      if (sub.submitted_data && typeof sub.submitted_data === 'object') {
        return Object.values(sub.submitted_data).some((val) => String(val).toLowerCase().includes(term));
      }
      return false;
    });
    ...
    <div className="space-y-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="text-sm text-slate-500">Export submissions to spreadsheet CSV logs and configure template dynamic fields.</p>
        </div>
      ...
      <div className="flex border-b border-slate-200">
        <button
          ...
          className={`flex items-center gap-2 px-6 py-3 text-sm font-semibold border-b-2 transition ${activeTab === 'submissions' ? 'border-crimson text-navy' : 'border-transparent text-slate-500 hover:text-slate-800'}`}
        >
        ...
        <button
          ...
          className={`flex items-center gap-2 px-6 py-3 text-sm font-semibold border-b-2 transition ${activeTab === 'forms' ? 'border-crimson text-navy' : 'border-transparent text-slate-500 hover:text-slate-800'}`}
        >
            ...
            <div className="overflow-hidden rounded-xl border border-slate-200 bg-white font-mono shadow-xl">
              {loadingSubmissions ? (
                ...
                <div className="flex flex-col items-center justify-center p-16 text-center">
                  ...
                  <p className="mt-2 max-w-sm text-xs text-slate-500">Submissions will display here once visitors submit forms on the college portal.</p>
                </div>
              )}
            </div>
          </div>
          ...
          <div className="space-y-4">
            <div className="rounded-xl border border-slate-200 bg-white font-mono overflow-hidden">
              <div className="divide-y divide-slate-200">
                {formsList.map((form) => (
                  <div key={form.id} className="flex items-center justify-between p-5 hover:bg-slate-50 transition">
                    <div>
                      <h4 className="text-sm font-semibold text-slate-800">{form.form_name}</h4>
                      <p className="text-[10px] text-crimson font-mono mt-0.5">FieldsCount: {Array.isArray(form.fields_config) ? form.fields_config.length : 0}</p>
                    </div>

                    <div className="flex items-center gap-2">
                      <button onClick={() => handleOpenEditForm(form)} className="rounded p-1.5 text-slate-500 hover:bg-slate-100 hover:text-navy">
                        <Edit2 className="h-4 w-4" />
                      </button>
                      <button onClick={() => handleDeleteForm(form)} className="rounded p-1.5 text-slate-500 hover:bg-rose-500/15 hover:text-rose-400">
                        <Trash2 className="h-4 w-4" />
                      </button>
                    </div>
                  </div>
      ...
      {isFormModalOpen && (
        <div className="fixed inset-0 flex items-center justify-center bg-slate-950/80 p-4 z-50">
          <div className="w-full max-w-lg rounded-lg border border-slate-200 bg-white p-6 shadow-2xl">
            <h3 className="font-display text-lg font-bold text-navy mb-4">{editingForm ? 'Edit Form Template Details' : 'Create Inquiry Form Template'}</h3>
            ...
            <form onSubmit={handleSaveForm} className="space-y-4">
              ...
              <div className="flex justify-end gap-3 pt-4 border-t border-slate-200">
                <button type="button" onClick={() => setIsFormModalOpen(false)} className="rounded border border-slate-200 px-4 py-2 text-xs text-slate-500 hover:text-navy">
                  Cancel
                </button>
