...
function ScopeFields({
  ...
  const entityOptions = scopeType === 'trust' ? options.trusts : scopeType === 'college' ? options.colleges : scopeType === 'department' ? options.departments : [];
  ...
  return (
    <>
      ...
      <div className="space-y-1.5">
        ...
        <select value={roleCode} onChange={(e) => setRoleCode(e.target.value)} className="w-full rounded border border-slate-200 bg-white px-3 py-2 text-sm text-slate-900 focus:outline-none focus:ring-2 focus:ring-crimson/30">
      ...
      {entityLabel && (
        <div className="space-y-1.5">
          ...
          <select value={scopeId} onChange={(e) => setScopeId(e.target.value)} className="w-full rounded border border-slate-200 bg-white px-3 py-2 text-sm text-slate-900 focus:outline-none focus:ring-2 focus:ring-crimson/30">
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between gap-4">
        ...
        <button onClick={() => setCreateOpen(true)} className="inline-flex items-center gap-2 rounded-lg bg-crimson px-4 py-2.5 text-sm font-semibold text-white hover:bg-crimson/90 transition">
        ...
        {loading ? (
          ...
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-slate-100">
              {users.map((u) => (
                <tr key={u.id}>
                  <td className="px-4 py-3 text-slate-900 font-medium">{u.firstName || u.lastName ? `${u.firstName} ${u.lastName}`.trim() : <span className="text-slate-400">—</span>}</td>
                  ...
                  <td className="px-4 py-3">
                    <div className="flex items-center justify-end gap-1">
                      ...
                      <button title="Manage roles" onClick={() => setRolesUser(u)} className="rounded p-1.5 text-slate-500 hover:text-navy hover:bg-slate-100">
      ...
      <form onSubmit={handleSubmit} className="space-y-4 max-h-[70vh] overflow-y-auto pr-1">
        <div className="grid grid-cols-2 gap-3">
          <div className="space-y-1.5">
            ...
            <input value={firstName} onChange={(e) => setFirstName(e.target.value)} className="w-full rounded border border-slate-200 bg-white px-3 py-2 text-sm text-slate-900 focus:outline-none focus:ring-2 focus:ring-crimson/30" />
          </div>
          <div className="space-y-1.5">
            <label className="text-xs font-semibold text-slate-900 uppercase tracking-wider">Last Name</label>
            <input value={lastName} onChange={(e) => setLastName(e.target.value)} className="w-full rounded border border-slate-200 bg-white px-3 py-2 text-sm text-slate-900 focus:outline-none focus:ring-2 focus:ring-crimson/30" />
          </div>
        </div>
        ...
        <div className="space-y-1.5">
          ...
          <div className="relative">
            ...
            />
            <button type="button" onClick={() => setShowPassword((v) => !v)} className="absolute right-2.5 top-1/2 -translate-y-1/2 text-slate-400 hover:text-slate-600" tabIndex={-1}>
              {showPassword ? <EyeOff className="h-4 w-4" /> : <Eye className="h-4 w-4" />}
            </button>
          </div>
        </div>

        <ScopeFields options={options} roleCode={roleCode} setRoleCode={setRoleCode} scopeType={scopeType} setScopeType={setScopeType} scopeId={scopeId} setScopeId={setScopeId} />
...
function EditProfileModal({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSaving(true);
    try {
      await updatePortalUserProfile({ userId: user.id, firstName, lastName });
  ...
  return (
    <ModalShell title={`Edit Profile — ${user.email}`} onClose={onClose}>
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <button type="submit" disabled={saving} className="w-full inline-flex items-center justify-center gap-2 rounded-lg bg-crimson px-4 py-2.5 text-sm font-semibold text-white hover:bg-crimson/90 transition disabled:opacity-50">
          {saving && <Loader2 className="h-4 w-4 animate-spin" />}
          Save Changes
        </button>
      </form>
...
function ManageRolesModal({
  ...
  const handleRemove = async (userRoleId: string) => {
    ...
    setSaving(true);
    try {
      await removePortalUserRole(userRoleId);
  ...
  return (
    <ModalShell title={`Manage Access — ${user.email}`} onClose={onClose}>
      <div className="space-y-2 mb-5">
        ...
        {currentRoles.map((r) => (
          <div key={r.userRoleId} className="flex items-center justify-between rounded border border-slate-200 bg-slate-50 px-3 py-2">
            <span className="text-sm font-semibold text-navy">
              {r.roleName} <span className="text-slate-500 font-normal">· {r.scopeLabel}</span>
            </span>
            <button onClick={() => handleRemove(r.userRoleId)} disabled={saving} className="text-slate-500 hover:text-crimson disabled:opacity-50">
...
function ResetPasswordModal({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSaving(true);
    try {
      await adminSetUserPassword({ userId: user.id, newPassword });
