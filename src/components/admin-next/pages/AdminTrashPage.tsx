...
import { Trash2, RefreshCw, Search, Loader2, Archive, ChevronLeft, ChevronRight } from 'lucide-react';
...
export function AdminTrashPage({ admin }: { admin: AdminUser }) {
  const supabase = useMemo(() => createClient(), []);
  const isAdmin = admin.roles.some((r) => r.code === 'admin');
  ...
  const handlePurgeConfirm = async (recordId: string) => {
    ...
    setPurgeConfirmId(null);
    try {
      const { error } = await (supabase as any).from(selectedTable).delete().eq('id', recordId);
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-slate-500">
          ...
          {!isAdmin && <span className="ml-2 rounded bg-amber-500/10 px-2 py-0.5 text-xs text-amber-400 border border-amber-500/20">Purge requires Global Admin role</span>}
        </p>
      </div>
      ...
      {purgeConfirmId && (
        <div className="flex items-center justify-between rounded-xl border border-rose-500/40 bg-rose-500/5 p-4">
          ...
          <div className="flex gap-2 ml-4 shrink-0">
            <button onClick={() => setPurgeConfirmId(null)} className="rounded border border-slate-200 px-3 py-1.5 text-xs text-slate-500 hover:text-navy transition">
              Cancel
            </button>
            <button onClick={() => handlePurgeConfirm(purgeConfirmId)} className="rounded bg-rose-600 px-3 py-1.5 text-xs font-bold text-navy hover:bg-rose-500 transition">
                ...
                {filteredRecords.map((r) => {
                  ...
                  return (
                    <tr key={r.id} className="hover:bg-slate-100 transition">
                      ...
                      <td className="px-6 py-4 text-xs text-slate-700 font-semibold font-mono">{r.deleted_at ? new Date(r.deleted_at).toLocaleString() : 'Unknown'}</td>
                      <td className="px-6 py-4 text-xs text-slate-600 font-mono">{r.id}</td>
                      <td className="px-6 py-4 text-right">
                        <div className="flex justify-end gap-3">
                          ...
                          >
                            {restoringId === r.id ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <RefreshCw className="h-3.5 w-3.5" />}
                            <span>Restore</span>
                          </button>
