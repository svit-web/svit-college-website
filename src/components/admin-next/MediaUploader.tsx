...
export function MediaUploader({ value, onChange, type = 'image', bucketName = 'media' }: MediaUploaderProps) {
  ...
  return (
    <div className="space-y-2">
      <input ref={fileInputRef} type="file" accept={type === 'image' ? 'image/*' : '*'} onChange={handleFileChange} className="hidden" />
        ...
        <div className="relative rounded-lg border border-slate-200 bg-slate-50 p-2">
          <div className="flex items-center gap-3">
            ...
            <div className="flex-1 min-w-0">
              ...
              <a href={value} target="_blank" rel="noreferrer" className="inline-flex items-center text-xs font-semibold text-crimson hover:text-crimson/80 mt-1">
                View Uploaded Asset
              </a>
            </div>

            <button type="button" onClick={handleRemove} className="rounded-full bg-slate-100 p-1 text-slate-600 hover:bg-red-50 hover:text-red-500 transition" title="Remove file">
              <X className="h-4 w-4" />
            </button>
          </div>
        </div>
        ...
        <div className="space-y-2">
          <div className="inline-flex rounded-lg bg-slate-100 p-1 text-xs font-semibold">
            <button
              ...
              className={`rounded-md px-3 py-1 transition ${mode === 'upload' ? 'bg-white text-navy shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}
            >
              Upload File
            </button>
            <button
              type="button"
              onClick={() => setMode('url')}
              className={`rounded-md px-3 py-1 transition ${mode === 'url' ? 'bg-white text-navy shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}
            >
              Paste URL
            </button>
          </div>
          ...
          {mode === 'upload' ? (
            <div
              ...
              className={`flex flex-col items-center justify-center rounded-lg border-2 border-dashed p-6 text-center cursor-pointer transition ${
                dragActive ? 'border-crimson bg-crimson/5 text-crimson' : 'border-slate-300 bg-slate-50 text-slate-600 hover:border-slate-400 hover:bg-slate-100'
              }`}
            >
              {uploading ? (
                ...
                <>
                  ...
                  <p className="text-xs text-slate-500 mt-1">Supports {type === 'image' ? 'PNG, JPG, WEBP' : 'PDF, DOCX, ZIP'} up to 10MB</p>
                </>
              )}
            </div>
          ) : (
            <div className="flex gap-2">
              ...
              />
              <button type="button" onClick={handleUrlSubmit} className="shrink-0 rounded-lg bg-crimson px-3 py-2 text-xs font-semibold text-white hover:bg-crimson/90 transition">
                Use URL
              </button>
            </div>
          )}
