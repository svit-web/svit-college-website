...
export function AdminHeader({ profile, roles, logout, onMobileMenuToggle }: HeaderProps) {
      ...
      return (
        <div key={path} className="flex items-center">
          ...
          {isLast ? (
            ...
            <Link to={to} className="text-slate-500 hover:text-navy transition font-medium text-xs">
              {label}
            </Link>
          )}
        </div>
    ...
    <header className="flex h-14 w-full shrink-0 items-center justify-between border-b border-slate-200 bg-white px-4 shadow-sm">
      <div className="flex items-center gap-2">
        ...
        <div className="flex items-center">
          <Link to="/admin" className="text-slate-500 hover:text-navy transition font-medium text-xs hidden sm:block">
            Admin
          </Link>
        ...
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            <button className="flex items-center gap-2 rounded-full border border-slate-200 bg-slate-50 p-1 pr-3 text-navy hover:bg-slate-100 hover:border-slate-300 transition">
              ...
              <span className="max-w-[90px] truncate text-xs font-semibold text-navy hidden sm:block">
                {userFullName}
              </span>
            </button>
          </DropdownMenuTrigger>
