...
import { courses, engDepts, recruiters } from "@/data/site";
...
function CoursePage() {
  ...
  const isEng = course.slug === "engineering";
  return (
    <>
          ...
          <aside className="space-y-4">
            <div className="rounded-2xl bg-gradient-to-br from-navy to-navy-light p-6 text-white">
              ...
              <dl className="mt-4 space-y-3 text-sm">
                <div><dt className="text-white/60 text-xs">Full Name</dt><dd>{course.fullName}</dd></div>
                ...
                <div><dt className="text-white/60 text-xs">Intake</dt><dd>{course.intake}</dd></div>
              </dl>
            ...
            <div className="rounded-2xl border border-border bg-white p-6">
              ...
              <ul className="mt-3 space-y-2 text-sm">
                <li><Link to="/courses/$course/faculty" params={{ course: course.slug }} className="flex items-center gap-2 text-navy hover:text-gold"><Users className="h-4 w-4" /> Faculty</Link></li>
      ...
      {isEng && (
        <section className="bg-secondary/50 py-20">
          <div className="container-page">
            <SectionHeading center eyebrow="Engineering Departments" title="12 Specialised Branches" />
            <div className="mt-12 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
              {engDepts.map((d, i) => (
                <Reveal key={d.slug} delay={i * 0.03}>
                  <Link to="/courses/engineering/$dept" params={{ dept: d.slug }} className="card-lift group flex h-full flex-col rounded-2xl border border-border bg-white p-6">
                    <div className={cn("mb-3 flex h-10 w-10 items-center justify-center rounded-md text-white text-xs font-bold", d.color)}>{d.short}</div>
                    <h4 className="font-display font-bold text-navy">{d.name}</h4>
                    <p className="mt-2 text-sm text-muted-foreground line-clamp-3">{d.overview}</p>
                  </Link>
                </Reveal>
              ))}
            </div>
