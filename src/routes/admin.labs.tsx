...
interface Highlight { title: string; description: string }
interface Lab {
  id: string;
  name: string;
  slug: string;
...
function AdminLabsPage() {
  ...
  const [form, setForm] = useState<{
    name: string; slug: string; department_id: string;
    subtitle: string; accent: string; description: string;
    highlights: Highlight[]; imageUrl: string; status: string;
  }>({
    name: "", slug: "", department_id: lockedDeptId ?? "",
    subtitle: "", accent: "", description: "",
    highlights: [], imageUrl: "", status: "published",
  });
  ...
  function openNew() {
    setEditingId(null);
    setForm({
      name: "", slug: "", department_id: lockedDeptId ?? selectedDeptId,
      subtitle: "", accent: "", description: "",
      highlights: [], imageUrl: "", status: "published",
    });
    setPanelOpen(true);
  ...
  async function handleSave(e: React.FormEvent) {
    e.preventDefault();
    if (!form.department_id) { toast.error("Select a department first."); return; }
    setSaving(true);
    try {
      const payload = {
        name: form.name.trim(),
  ...
  async function handleDelete(lab: Lab) {
    ...
    const { error } = await supabase.from("facilities")
      ...
      .eq("id", lab.id);
    if (error) toast.error(error.message);
    else { toast.success("Moved to trash."); fetchLabs(selectedDeptId); }
  }
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="mt-0.5 text-xs text-slate-500">
            {lockedDeptId
              ? `Showing labs for your department`
              : "Manage labs across all departments"}
          </p>
        </div>
        <button
          onClick={openNew}
      ...
      {/* Department selector (global admins only) */}
      {isGlobal && (
      ...
      {/* For locked dept_admin: show dept name */}
      {!isGlobal && departments.length > 0 && (
      ...
      {!selectedDeptId ? (
        ...
        <div className="flex h-48 flex-col items-center justify-center rounded-xl border border-dashed border-slate-200 bg-white text-center">
          ...
          <button onClick={openNew} className="mt-2 text-xs text-crimson hover:underline">Add first lab →</button>
        </div>
      ) : (
          ...
          {filtered.map((lab) => (
            ...
            >
              ...
              <div className="mt-3 flex items-center justify-between">
                ...
                <button
                  onClick={(e) => { e.stopPropagation(); handleDelete(lab); }}
        ...
        <>
          ...
          <div className="fixed inset-y-0 right-0 z-50 flex w-full max-w-lg flex-col bg-[#16181d] shadow-2xl border-l border-zinc-800 overflow-hidden">
            ...
            <form onSubmit={handleSave} className="flex-1 overflow-y-auto p-5 space-y-4 admin-scroll">
              {/* Department (locked for dept_admin) */}
              {isGlobal ? (
              ...
              <div className="space-y-2">
                ...
                {form.highlights.length === 0 && (
                  <p className="text-xs text-zinc-600">No highlights yet.</p>
                )}
                {form.highlights.map((hl, i) => (
                  <div key={i} className="flex gap-2 items-start rounded-lg border border-zinc-800 bg-zinc-900/50 p-3">
                    <div className="flex-1 space-y-2">
                      ...
                      <input type="text" value={hl.description}
