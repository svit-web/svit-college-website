...
import { Plus, Edit2, Trash2, Loader2, ShieldAlert, GraduationCap } from "lucide-react";
...
function AdminScholarshipsPage() {
  ...
  if (!loading && !isAdmin) {
    return (
      <div className="rounded-xl border border-amber-500/20 bg-amber-500/5 p-6">
        ...
        <p className="mt-1 text-sm text-slate-500">Only global admins can manage scholarships.</p>
      </div>
    );
  }
  ...
  async function handleSave() {
    ...
    setSaving(true);
    try {
      await upsertScholarship({ data: editing });
                  ...
                  <td className="p-3">
                    <div className="flex items-center gap-1.5 justify-end">
                      <button
                        ...
                        className="rounded-md p-1.5 text-slate-400 hover:bg-slate-100 hover:text-navy transition"
                        title="Edit"
                      >
