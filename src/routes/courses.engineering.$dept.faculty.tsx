...
function DeptFacultyPage() {
  ...
  const faculty = getFaculty(dept.slug, dept.careers.concat(dept.labs), 9);
  return (
    <>
      <PageHero title={`${dept.name} Faculty`} accent={dept.short} crumbs={[{ label: "Home", to: "/" }, { label: "Engineering" }, { label: dept.name, to: "/courses/engineering/$dept" }, { label: "Faculty" }]} />
      <section className="container-page py-20">
        ...
        <div className="mt-10 text-center">
          <Link to="/courses/engineering/$dept" params={{ dept: dept.slug }} className="inline-flex items-center gap-1 text-sm text-navy hover:text-gold">
