...
export const Route = createFileRoute("/courses/engineering/$dept")({
  loader: async ({ params }) => {
    const [dept, allDepts] = await Promise.all([
      getEngDeptBySlug({ data: params.dept }),
      getEngDepts(),
    ]);
    if (!dept) throw notFound();
    return { dept, others: allDepts.filter((d) => d.metadata.engSlug !== params.dept) };
  },
...
function DeptPage() {
  const { dept, others } = Route.useLoaderData();
  const m = dept.metadata;
  return (
    <>
      <PageHero title={dept.name} accent={`Engineering · ${m.short}`} subtitle={m.overview} crumbs={[{ label: "Home", to: "/" }, { label: "Engineering", to: "/courses/engineering" as any }, { label: dept.name }]} />
      ...
      <section className="bg-secondary/50 py-20">
        <div className="container-page">
          ...
          <div className="mt-10 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
            {others.map((d) => (
              <Reveal key={d.code}>
                <Link to="/courses/engineering/$dept" params={{ dept: d.metadata.engSlug }} className="card-lift group flex items-center gap-3 rounded-md border border-border bg-white p-4">
                  <div className={cn("h-9 w-9 shrink-0 rounded-md text-white flex items-center justify-center text-xs font-bold", d.metadata.color)}>{d.metadata.short}</div>
